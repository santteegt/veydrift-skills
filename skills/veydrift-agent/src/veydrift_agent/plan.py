...
5. **Non-combat fleet logistics** (Phase 5c, rung `8c`; Deploy and foreign Harvest added
   commits 3-4 of the launch-actions plan) — `candidates.select_logistics_candidate`:
   Transport between the player's own planets, then Deploy toward a declared
   `policy.strategy.fleet_home_planet_id`, then local Harvest, then foreign Harvest of a
   third party's debris field. Gated on `policy.actions.allow_fleet_noncombat` (default
   `false`, so this band produces nothing under the default policy) and reached only
   after bands 1-4 produced nothing at all for any target planet — an idle-capacity
   opportunity, never something that should outrank a scored economic pick, a
...
def plan_next_action(
    ...
    *,
    ...
    # this band produces nothing under the default policy). Reached only after bands 1-5
    # produced nothing at all for any target planet -- the most conservative placement in
    # the ladder, deliberately: colonizing is a permanent, high-stakes commitment of a
    # hard-to-replace asset, so it must never preempt routine economic activity, a
    # policy-declared research/ship/defense target, the unlock-chain rung, or ordinary
    # fleet logistics.
    colonize_winner, colonize_alternatives = candidates.select_colonize_candidate(
