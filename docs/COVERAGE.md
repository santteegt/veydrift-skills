...
### 1.1 Implemented (7 functions, 8 rows — `launchFleetMission`'s two overloads added 2026-08-17, Phase 5c/5b; `launchInterplanetaryMissileAttack` added 2026-08-28, launch-actions plan commit 7)
...
and by commit 7 (2026-08-28) to exclude a second, `launchInterplanetaryMissileAttack`
itself (moved to §1.1 below — it is now implemented). "Combat, alliances, ACS, migration,
referrals, NFT burns, the ERC-20 market bridge" remains the non-goal for everything below,
since none of these five *separate contract entrypoints* are reached via
`launchFleetMission`'s mission-type argument at all (nor, for the four here that remain
excluded, via any other write path this codebase implements) — each is its own selector,
and none of the five appear in `plan.py`, `guard.py`'s tier map, or `allowlist.ts`'s
signature lists (grepped, absent from all three). Enabling any of these five requires a
source change to `_MIN_TIER_FOR_FUNCTION` and `allowlist.ts`, unconditionally — no policy
flag reaches them.
...
see §1.1's `launchFleetMission` rows and `docs/SPEC.md` correction 70), and from
`launchInterplanetaryMissileAttack`'s own reachability, which commit 7 made real via a
wholly separate selector (not a `launchFleetMission` mission type at all — see §1.1's own
row for it and `docs/SPEC.md` correction 72). **Mission types 5 (AcsDefend) / 6
(Intercept) were later widened too**, per the ACS defense coordination feature
(`docs/SPEC.md` correction 74): reachable via `launchFleetMission` itself when the
DIFFERENT `policy.actions.allow_acs_defense` resolves `true` at tier 3
(`guard.py`'s `_ACS_MISSION_TYPES`, `allowlist.ts`'s `ACS_MISSION_TYPES`) —
`allow_combat` alone never unlocks these two. Mission types 8 (AcsAttack) / 9
(DefenseHold) stay outside both `launchFleetMission` mission-type sets unconditionally,
even at tier 3 — `launchAttackMission`/`joinAttackMission` below are the entrypoints for
AcsAttack specifically (via `joinAttackMission`) and remain fully out of scope regardless
of policy; DefenseHold(9) is dead enum space for `launchFleetMission` specifically
(structurally impossible via that function regardless of policy — confirmed against
`VeydriftGameplayModule.sol`'s own `uint8(missionType) > uint8(Intercept)` revert), its
own `launchDefenseHold` entrypoint below is the real, separate, now-implemented path.
...
| `launchDefenseHold` | declared `VeydriftGame.sol:460`, impl `VeydriftDefenseHoldModule.sol:58` | **implemented (ACS defense coordination feature, `docs/SPEC.md` correction 74)** — override-only, gated on `policy.actions.allow_acs_defense` at `operator` tier; not yet fork-verified | See `skills/veydrift-agent/references/coordination.md` |
...
| Attack/resolved-battle/debris radar | **implemented and live-verified (2026-09-02)** | `radar.py` (new module) — three independent signals: `incoming_fleet` (future arrivals, `/wallet/{addr}/fleet-visibility`), `resolved_attack` (past combat, `/wallet/{addr}/missions`'s `report`-bearing rows), `debris` (a tracked planet's own slot, `/universe/galaxies/{g}/systems/{s}`). Built specifically because `incoming_fleets` alone missed a real attack during this feature's own planning — see `references/radar.md`. Two entry points share one core (`radar.check_targets`): `vd tick` (`policy.radar.enabled`, default `true`, scoped to `policy.planets`) and standalone `vd radar check --wallet ADDR [--planets N] \| --alliance-id N` (no `policy.json` required; `--alliance-id` resolves every member's every planet via the previously-unwired `GET /alliance/{id}`). Read-only throughout — no `ActionKind`, no guard gate, no `veydrift-wallet` involvement. Live-verified against this project's own account and a real 6-member alliance (id 29); a genuine, unedited fixture (`tests/fixtures/wallet_missions_resolved_attack.json`) pins the real row shape after an initial implementation bug (filtering on a `kind` value that has never actually appeared on live data) was caught by that same live check. |
| Attack/missile/colonize/foreign-harvest opportunities, surfaced independent of the ladder | **implemented** | `opportunities.py` (new module) — `plan_next_action` (`plan.py`) is a straight-line early-return chain, so a lower-priority band's candidate (logistics, colonize, attack, missile) is never even generated once a higher band wins that tick, regardless of how long the relevant policy flag has been on. `attack_targets`/`missile_targets`/`foreign_debris_targets`/`colonize_targets` were already fetched tier-independently every tick, but the data derived from them was invisible on any tick where a higher-priority band won. `opportunities.py` calls the same four `candidates.py` generators (`generate_attack_candidates`/`generate_missile_candidates`/`generate_colonize_candidates`/`generate_foreign_harvest_candidates`) a second time, once per owned planet, independent of the ladder — no changes to `plan.py`/`candidates.py`, no new policy flag (each generator already self-gates on `allow_combat`/`allow_fleet_noncombat`/`strategy.colonize` internally), no persisted state (an opportunity is a current-state fact, not a one-time event needing de-duplication). `vd tick` only — no standalone CLI. See `references/opportunities.md`. |
...
| Alliances | **membership implemented and fork-verified (2026-09-01/02); ACS defense coordination (`openDefenseIntent`) implemented, not yet fork-verified — diplomacy remains out of scope** | `VeydriftAllianceSystem.sol` is its own deployed contract, its own pinned ABI (`abi/VeydriftAllianceSystem.202d1ac.json` + `abi/PINNED.alliance.json`, `skills/veydrift-wallet`; re-pinned to `202d1ac` on the 2026-09-07 game-contract upgrade — the alliance address is unchanged and the 15 membership selectors are byte-identical across the re-pin), its own address (`allianceContractAddress`, live in `/runtime-config`). 15 membership functions (`createAlliance`, `updateAllianceProfile`, `inviteMember`, `cancelInvite`, `acceptInvite`, `requestJoinAlliance`, `cancelJoinRequest`, `dismissJoinRequest`, `approveJoinRequest`, `kickMember`, `kickMembers`, `leaveAlliance`, `setMemberRole`, `setMembersRole`, `transferAllianceOwnership`) are reachable via `vd tick --action` only — no `candidates.py` generator, no `plan.py` rung — gated on `policy.actions.allow_alliance` at `economy` tier or above (`guard._gate_alliance_action`; `veydrift-wallet`'s `ALLIANCE_SIGNATURES` in `allowlist.ts`). `setDiplomacy` (Ally/NAP/War) remains fully out of scope, unconditionally — combat-adjacent, deferred. **`openDefenseIntent` is a 16th, override-only function on this same contract (ACS defense coordination feature, `docs/SPEC.md` correction 74)**, gated on the different `policy.actions.allow_acs_defense` flag at `economy` tier, kept out of `_ALLIANCE_FUNCTIONS`/`ALLIANCE_SIGNATURES` deliberately (its own `_ACS_ALLIANCE_FUNCTIONS`/`ACS_ALLIANCE_SIGNATURES` carve-out) — not yet exercised via fork-testing, see `launchDefenseHold`'s row below and `skills/veydrift-agent/references/coordination.md` for the honest verification-status summary shared by all four ACS defense coordination functions. **Live-sent on a local Anvil fork (round 5, 2026-09-02)**: 13 of the 15 membership functions, across a full lifecycle (create/invite/cancel/accept/request/dismiss/approve/role-change/ownership-transfer/kick/leave) using three fresh, alliance-free accounts, every step confirmed against real on-chain state, not just emitted events — `kickMembers`/`setMembersRole` (the two batch variants) are the only ones not live-sent, structurally identical to their already-verified singular siblings. See `docs/SPEC.md` correction 73 and `skills/veydrift-wallet/references/fork-testing.md` §12. |
