...
**What the spec explicitly does not attempt (§1's non-goals):** migration, referrals, NFT
burns, the ERC-20 market bridge, diplomacy, and any raid-profitability model —
`protectedResources`' actual semantics are unconfirmed, so nothing here builds loot logic
on it. Combat is almost entirely a non-goal too, with narrow exceptions:
...
**Alliances are no longer a non-goal either, as of the alliance feature (2026-09-01)**: 15
membership functions on `VeydriftAllianceSystem` (a wholly separate deployed contract) are
reachable via `vd tick --action` only — never planner-proposed, no ladder rung — gated on
`policy.actions.allow_alliance` at `economy` tier or above. **Neither is ACS defense
coordination, as of that feature (see §5/§7 below)**: AcsDefend(5)/Intercept(6)
(`launchFleetMission`), `launchDefenseHold` (its own entrypoint), and `openDefenseIntent`
(on the same alliance contract, a 16th function) are reachable the same manual-override-
...
| Module | Lines | Role |
| --- | --: | --- |
| `models.py` | 601 | **Frozen.** Every pydantic model — `Policy`, `Action`, `Snapshot`, `GuardReport`, `UnsignedTx` — is the on-disk JSON shape for `policy.json`, both log files, and the generated schemas. Treat a field rename here as a breaking change to every log line ever written. The launch-actions plan (2026-08-28) added two fields under this same "frozen means additive, not immutable" discipline: `Action.target_planet_id` (a foreign Harvest target's real on-chain id, since a foreign planet is never in `Snapshot.planets` for the existing `target_coordinates` lookup to resolve) and `StrategyCfg.colonize`/`fleet_home_planet_id` (gate Colonize/Deploy respectively) — both additive, both regenerated into `schemas/`. **The alliance feature (2026-09-01)** added `ActionKind.ALLIANCE`, `ActionsCfg.allow_alliance`, a new `Action` field block (`alliance_id`/`target_player(s)`/`role`/`alliance_tag`/`alliance_name`/`alliance_description`), and a new `AllianceState` model group (`AllianceMembership`/`AllianceMember`/`AlliancePendingInvite`/`AlliancePendingJoinRequest`/`AllianceJoinRequestForOwner`/`AllianceDirectoryEntry`) — guard-time-only live data, deliberately never added to the frozen `Snapshot` itself, the same posture `attack_protection_allowed` already takes. **The ACS defense coordination feature** added `ActionKind.DEFENSE_HOLD` (shares nothing with `FLEET_MISSION` — no `mission_type` argument at all, the same "wholly separate entrypoint, own `ActionKind`" precedent `MISSILE_ATTACK` set) and `Action.hold_seconds`; widened `Action.mission_id`'s existing docstring to also carry the *hostile* mission id for AcsDefend/Intercept/`openDefenseIntent` (never `target_planet_id`, a deliberate design choice keeping the on-chain `targetPlanetId`-repurposing an encoding-time-only concern in `tick.py`); added `ActionsCfg.allow_acs_defense`; added `RadarFinding.mission_id`/`mission_type_name` (previously discarded by `radar.py`, now what `coordination.py` reads); added `CoordinationSuggestion`/`CoordinationReport`; and widened `OpportunityFinding.family`'s `Literal` to include `"transport"`. All additive, all regenerated into `schemas/` where schema-backed (`Policy`/`Action` only — `RadarFinding`/`CoordinationSuggestion`/`OpportunityFinding` aren't). |
...
| `alliance_ids.py` | ~70 | **New, alliance feature.** `AllianceRole` (None/Member/Officer/Owner), from `VeydriftAllianceSystem.sol` — a genuinely different deployed contract, so this is a deliberate sibling module to `ids.py`, not an addition to it (`ids.py`'s own docstring scopes itself to the game contract's six enums specifically). Same citation/lookup conventions, plus `meets_min_role()`. |
...
| `read.py` | 993 | One `vd read` subcommand per API route (18 targets), plus several general-purpose fetchers that bypass the CLI layer entirely (`fetch_universe_system`, `fetch_raid_finder_debris`, `fetch_attack_protection`, `fetch_alliance_state` — alliance feature — and, for the radar feature, `fetch_alliance_by_id` (`GET /alliance/{id}`, previously unwired) and `fetch_missions` — all the same "raw dict, no `vd read` target" posture `fetch_fleet_visibility`/`fetch_activity` already used). `--summary` mode is the default and is capped near 2KB; `battle-reports`/`highscores` refuse to print to stdout at all — `--out` is mandatory, since they run 60KB–2.2MB uncompressed. |
...
| `guard.py` | ~2000 | **25** guardrail gates (17 through Phase 4; Phase 5c added `mission_type`; a later addition added `game_paused`; the launch-actions plan added `fleet_slots` (commit 2), `attack_protection` (commit 6), and `missile_target` (commit 7), 2026-08-28; the alliance feature added `alliance_action`, 2026-09-01; the ACS defense coordination feature added `acs_defend_target`/`defense_hold_target`), every one evaluated and reported on every call — never short-circuited, so a passing tick's verdict list is as informative as a blocked one. **`alliance_action`** independently re-derives every one of the 15 `VeydriftAllianceSystem` membership functions' preconditions (caller role floor, membership/invite/request-row lookups, batch-fails-closed for `kickMembers`/`setMembersRole`) from a fresh live `alliance_state` fetch — this contract has no `Snapshot` equivalent, so nothing here is checkable offline the way `missile_target` is. `_gate_abi_hash` gained an alliance branch that PASSes unconditionally (no live-hash verification path exists for that contract's ABI, ever). `idempotency_key` gained a fourth special case for the same `planet_id`/`entity_id`-always-`None` collision `FLEET_MISSION`/`RESOLVE_MISSION`/`MISSILE_ATTACK` were each fixed for previously. The rule every gate follows: missing data resolves toward `BLOCK`/`ESCALATE`, never `PASS`. **Fix, then completed:** `health` gained a narrow exception, `Snapshot.combat_only_degradation()` — `/health`'s `ok:false` caused *solely* by a combat-only `randomnessReadiness` degradation (positively confirmed, everything else on the snapshot fine) PASSes instead of BLOCKing for a non-combat action. Commit 5 made this exception's premise narrower (Attack became conditionally reachable via `policy.actions.allow_combat`) without yet changing its behavior; **commit 6 completed the correction**: `_gate_health` now takes `action` as a parameter and withdraws the exception specifically for a combat (Attack) action, which requests VRF at launch and cannot resolve while randomness is degraded — every non-combat action still gets the exception, unchanged. Confirmed live and persistent, served via HTTP 503 — `read._fetch_or_exit()` now defensively recovers a parseable `/health` 5xx body (narrowly scoped to that one route) instead of hard-aborting before this check could ever run. **The launch-actions plan** also fixed idempotency-key collisions in `idempotency_key` (every fleet mission from one planet, every resolve action across all planets, and — commit 7 — every missile launched from one planet each shared a single key and revert-streak counter), closed an in-flight-Colonize blind spot in `_colony_cap_violation` (`Snapshot.owned_planet_count` alone only reflects already-resolved planets; a new `outgoing_colonize_count` parameter, fed by `tick._outgoing_colonize_count`, folds in still-`Outbound` Colonize missions before the cap check, failing closed on `None` exactly like the field it complements), added `attack_protection` (commit 6, extended to Missile in commit 7 with a bashing-limit exemption — `launchInterplanetaryMissileAttack` calls `_enforceAttackProtection` with `countsBashing=false`): a live, target-specific `/wallet/{addr}/attack-protection` re-check for an Attack or Missile action — `None`/unknown **BLOCK**s, `true` **PASS**es, `false` **BLOCK**s unless the action is Missile and `blockedReason=="bashing"`; and added `missile_target` (commit 7): independently re-derives `launchInterplanetaryMissileAttack`'s range/primary-target/owned-count preconditions from `Snapshot` alone, no live data needed. **The ACS defense coordination feature** added `acs_defend_target` (AcsDefend/Intercept — exactly-`Attack` hostile-mission-type check, still-`Outbound` status, target match, the 5-minute join cutoff, `FleetAlreadyArrived`, origin ownership, and the live `coordination_allowed` re-check; a live `coordination_allowed=true` is never sufficient alone, since `VeydriftAllianceSystem._canCoordinateDefense` short-circuits `true` for a self-owned defended planet before checking any of the above) and `defense_hold_target` (`launchDefenseHold` — `hold_seconds` bounds `[3600, 115200]`, same-planet rejection, origin ownership, live `coordination_allowed`, which *is* sufficient alone here). `alliance_action` gained a 16th function, `openDefenseIntent`, gated on the different `allow_acs_defense` flag and kept out of the existing alliance-function set (a new, separate `_ACS_ALLIANCE_FUNCTIONS` carve-out, so the pre-existing alliance cross-layer equality never breaks). `_derive_fleet_mission_spend` now folds in the live `netHoldingFuelCost` for both new action shapes (never a recomputed formula) — the same understated-spend defect class judge finding 1 already fixed once for the plain `FLEET_MISSION` case. `idempotency_key` gained a fifth special case (`DEFENSE_HOLD`) and an `openDefenseIntent` sub-case inside the `ALLIANCE` branch; the existing `FLEET_MISSION` branch's key now folds in `mission_id`, closing a real collision between two different AcsDefend/Intercept actions from the same planet against different hostile missions. A 15-defect adversarial review pass (Fable 5) against this feature's own draft plan, before any code was written, is incorporated in full — see `docs/SPEC.md` correction 74. |
| `state.py` | 350 | `$VEYDRIFT_HOME` resolution, `AgentState` (pending txs, cumulative gas, revert counts), the tick lockfile, `KILLSWITCH` detection. Radar feature: a sibling `RadarState`/`radar-state.json`, deliberately not folded into `AgentState` — that model is implicitly single-wallet-shaped, but `vd radar check --alliance-id` watches many wallets that aren't `policy.wallet` at all, so the new state is keyed by wallet address instead. |
| `tick.py` | ~2700 | The orchestrator — the nine-step loop (§7), the `walletctl` subprocess bridge, `--readiness`, `--action`'s manual-override substitution point. The largest module in the package, and where the criticals a first-pass judge review found actually lived (`AGENTS.md` §5's unit-mismatch and revert-recording invariants). The launch-actions plan added six out-of-band fetchers here, each following the same established pattern (`_resolvable_mission_ids`'s "bypass the frozen `Snapshot`, best-effort, never raise" contract): `_own_planet_debris`, `_foreign_debris_targets`, `_colonize_targets`, `_outgoing_colonize_count` (commits 1-4), and `_attack_targets`/`_missile_targets` (commits 6-7, both gated on `policy.actions.allow_combat`). One live-data fetcher in this family is NOT a courtesy/generation-time read: `_attack_protection_allowed` (commit 6, extended to return `(allowed, blocked_reason)` in commit 7) feeds `guard._gate_attack_protection` directly, fetched fresh at guard-evaluation time for the specific resolved target of an Attack or Missile proposal. `_action_to_walletctl_json` also gained a `launchInterplanetaryMissileAttack` branch (commit 7) — not overloaded on the deployed ABI, so it's resolved by bare name, unlike `launchFleetMission`'s two forms. **The alliance feature** added `_alliance_state` (fetched whenever `policy.actions.allow_alliance` is true, regardless of what action that tick resolves to — unlike every other fetcher here, this one also drives the tick report's new "alliance:" line, not just guard evaluation), a required `_live_addresses()` fix (`allianceContractAddress`, or `guard._gate_address` would spuriously BLOCK every alliance tx), and 15 new `_action_to_walletctl_json` branches via a new `_ALLIANCE_ARG_BUILDERS` dispatch table — every one sets `"contract": "alliance"`, the field `veydrift-wallet`'s `buildTx` reads to resolve both the ABI and the destination address. **The radar feature** builds `radar.WatchTarget`s directly from the `Snapshot` already fetched this tick (`radar.targets_from_planet_snapshots`, no extra `/wallet/{addr}/planets` call), calls the shared `radar.check_targets`, and folds the resulting `RadarReport` into the tick report's new "Radar" line and — when findings are non-empty — an unconditional `strategy.md` entry, gated on `policy.radar.enabled` (default `true`, see `radar.py`'s row above for why). **The opportunities feature** calls `opportunities.scan_opportunities` unconditionally, right after `attack_targets`/`missile_targets`/`foreign_debris_targets`/`colonize_targets` are fetched for the ladder — no `if` needed, since every generator/selector it calls already self-gates on its own flag or real-time precondition internally. Folds into the report's new "opportunities:" line via `_opportunity_summary_line`, deliberately *without* radar's unconditional `strategy.md` append (see `opportunities.py`'s row above for why that distinction matters). Since the ladder-band survey addition, this line can now also diagnose Band 2 (building) winning nearly every tick and starving research/ships/defense — the opposite direction from the original per-planet families, which diagnose a lower band the ladder never reaches. **The ACS defense coordination feature** added `_hostile_mission_fetch` (`GET /mission/{id}`, best-effort) and three live pre-check probe functions (`_acs_defend_coordination_probe`/`_defense_hold_coordination_probe`/`_open_defense_intent_coordination_probe`), each building+simulating one of `VeydriftAllianceSystem`'s view functions (`counterplayDefenseFuelContext`/`defenseHoldFuelContext`/`canCoordinateDefense`) via a new `walletctl simulate --json` capability — a deliberately SEPARATE `_walletctl_simulate_probe` function, not a widening of the existing, well-tested `_walletctl_simulate` the pre-send path already calls. `_action_to_walletctl_json` gained branches for AcsDefend/Intercept (repurposing `action.mission_id` into the `targetPlanetId` calldata slot — AGENTS.md §7's third trap), `launchDefenseHold` (GAME contract, no `contract` key), and `openDefenseIntent` (`contract: "alliance"`). New `coordination_report = coordination.suggest_coordination(radar_report)`, computed right after `radar_report`, gated on `policy.actions.allow_alliance` (a visibility gate, independent of `allow_acs_defense`) — folds into the tick report's new "coordination:" line and, when non-empty, an unconditional `strategy.md` entry per suggestion (same never-suppressed treatment radar's own findings get, for the same time-bounded-urgency reason). |
...
| `radar.py` | ~370 | **New.** Attack/resolved-battle/debris radar — read-only throughout, no `ActionKind`, no guard gate, no `veydrift-wallet` involvement. Built directly from a live incident during this feature's own planning: `snapshot.incoming_fleets` alone missed a real attack because that field only ever lists future arrivals, never resolved ones. Three independent signals in one shared core (`check_targets`): `incoming_fleet` (every `incoming[]` row reported, not filtered to the hardcoded `hostile` flag — a non-Attack mission targeting your planet is itself diagnostic), `resolved_attack` (new — `/wallet/{addr}/missions`' `report`-bearing rows, de-duplicated per wallet against `radar-state.json`), `debris` (a tracked planet's own slot, independently re-reading the same route `tick._own_planet_debris` uses rather than reusing that `Snapshot`-shaped function directly, since a radar target can name a wallet this process has no `Snapshot` for at all). Two entry points share that core: `vd tick` (`policy.radar.enabled`, new `RadarCfg`, default `true` — a deliberate departure from this codebase's usual off-by-default convention, justified because radar is read-only) and standalone `vd radar check --wallet | --alliance-id`, exit code `0`/`1`/`2` (clean/findings/could-not-complete) as the actual notification contract, since no notification mechanism exists anywhere in this codebase. A real implementation bug — filtering resolved-attack rows on `kind == "battleReport"`, per the documented-but-actually-unobserved half of `/wallet/{addr}/missions`' tagged union — was caught by re-running the live check against this project's own account mid-implementation; see `references/radar.md`. **The ACS defense coordination feature** populates two previously-discarded fields on `incoming_fleet` findings, `RadarFinding.mission_id`/`mission_type_name` — a two-line addition, no new fetch — specifically so `coordination.py` has a real, usable `hostile_mission_id` to build a suggestion from instead of `None`. |
| `opportunities.py` | ~230 | **New.** Ten families, surfaced independent of `plan.py`'s ladder outcome, in two directions. `plan_next_action` is a straight-line early-return chain — a lower-priority band's candidate is never even generated once a higher band wins that tick, regardless of how long the relevant policy flag has been on. Five per-planet families (`attack`/`missile`/`colonize`/`foreign_harvest`/`transport`) call the same `candidates.py` generators a second time, once per owned planet via `plan._target_planets` (reused, not reimplemented), reporting every viable candidate. Five more, added alongside `policy.strategy.planet_rotation`, diagnose the opposite direction: Band 2 (building) unconditionally precedes Bands 3-4 (research, shipyard/defense, unlock-chain) with no policy-configurable weight between them, so an idle-queue economy can win the building rung essentially every tick, starving the rest indefinitely. `_scan_ladder_bands` calls the same `select_*` functions `plan.py` itself calls (`select_storage_candidate`/`select_building_candidate`/`select_research_candidate`/`select_shipyard_candidate`/`select_unlock_chain_candidate`) and reports only the winner each would pick right now, replicating plan.py's own external queue-state preconditions where the selector doesn't already self-gate on one (building: queue-empty per planet; research: account-wide queue empty) — see `opportunities._scan_ladder_bands`'s own docstring. Zero changes to `plan.py`/`candidates.py`/`guard.py` either way: every generator/selector already self-gates on its own policy flag or real-time precondition, so this module needs no gating logic of its own. No persisted state (an opportunity is a current-state fact, not a one-time event) and no unconditional `strategy.md` narration (unlike radar's findings, which are naturally transient, an opportunity can stay true for many ticks — forcing a line every tick would spam the log for something that hasn't changed); the tick report's `opportunities:` line and `proposals.jsonl`'s `"opportunities"` key are the only new surfaces. `vd tick` only, no standalone CLI. **The ACS defense coordination feature added the `transport` family**, via a dedicated `_scan_transport` call (`generate_transport_candidates` takes `target_planets` as a required positional 4th argument, unlike the other four per-planet generators' keyword-only shape, so it doesn't fit `_scan_planet`'s dispatch dict) — Transport's write path already existed and needed no new capability, only surfacing; Deploy remains excluded. See `references/opportunities.md`. |
| `coordination.py` | ~90 | **New.** ACS defense coordination suggestions, derived from `radar.py`'s own `incoming_fleet` findings. `suggest_coordination(radar_report) -> CoordinationReport` filters to findings whose `mission_type_name == "Attack"` exactly (the same requirement `guard._gate_acs_defend_target`'s own launch-path re-check enforces), and builds one `CoordinationSuggestion` per match, carrying the real `hostile_mission_id`. Zero new network calls, zero persisted state, zero effect on `plan.py`'s ladder or `guard.py`'s gates — same shape decision `opportunities.py` already made. States plainly that it cannot detect whether an `openDefenseIntent` has already been opened for a given hostile mission (no read of `AllianceDefenseIntentOpened` anywhere in this codebase) — a suggestion is re-surfaced every tick the hostile mission stays live and reachable, identically, regardless of whether it was already acted on. Wired into `vd tick` (gated on `policy.actions.allow_alliance`) and unconditionally into `vd radar check`. See `references/coordination.md`. |
...
| Module | Lines | Role |
| --- | --: | --- |
| `abi.ts` | 225 | Loads the pinned ABI, resolves a function by **full canonical signature** (never a bare name — `launchFleetMission` is overloaded on the deployed contract), computes selectors. **The ACS defense coordination feature** added `decodeSimulateReturnData(selector, returnData)` — decodes a `view` function's return data against its own ABI outputs, `undefined` when the function has no outputs or decoding fails; a generalizable capability, not specific to this feature's own three view functions. |
| `allowlist.ts` | ~330 | `checkAllowlist` — five checks, always all five evaluated and reported: destination against a **live** `/runtime-config` fetch, selector against the tier's set (computed from the pinned ABI, never a hand-typed hex constant), `value == 0`, `chainId == 8453`, and at `operator` tier, the `launchFleetMission` mission-type argument decoded from calldata and restricted to `OPERATOR_ALLOWED_MISSION_TYPES` (Transport/Deploy/Harvest, plus Colonize (2) as of Phase 5b) **unconditionally, or `COMBAT_ALLOWED_MISSION_TYPES` (Attack (3) only) when `resolveAllowCombat` resolves `policy.json`'s `actions.allow_combat` to `true`** (launch-actions plan commit 5, 2026-08-28 — no CLI flag or env var can assert this, unlike `resolveTier`'s own fallback). `guard.py`'s matching `mission_type` gate mirrors this exact two-set split. **Commit 7 (same date) adds a second, structurally different conditional path**: `launchInterplanetaryMissileAttack` (its own selector, sharing nothing with `launchFleetMission`) is pulled out of `tierSelectors('operator')`'s unconditional set entirely — via a new `COMBAT_SIGNATURES`/`combatSelectorSet()` — and checked the same lazy way inside the selector check itself, since there's no shared function to decode a conditional argument from the way Attack's mission type is. `guard.py`'s `_MIN_TIER_FOR_FUNCTION` maps the function unconditionally to `operator` instead (the `allow_combat` check lives in a separate `_gate_missile_target`); `test_tier_map_agrees_with_the_wallet_engines_allowlist` accounts for this shape difference via a new `guard._COMBAT_ONLY_FUNCTIONS` constant, diffed against `COMBAT_SIGNATURES` directly. **The ACS defense coordination feature adds a third conditional path, at `operator` tier, and a fourth at `economy`-or-above**: a new `ACS_MISSION_TYPES` set (AcsDefend(5)/Intercept(6)) is unioned into `launchFleetMission`'s allowed mission types when `resolveAllowAcsDefense()` resolves true — mirroring `guard.py`'s own `_ACS_MISSION_TYPES`, and deliberately never unlocked by `allow_combat` alone; `launchDefenseHold` gets its own `DEFENSE_HOLD_SIGNATURES`/`defenseHoldSelectorSet()` pair, checked the same lazy way `COMBAT_SIGNATURES` is, at `operator`; `openDefenseIntent` gets its own `ACS_ALLIANCE_SIGNATURES`/`acsAllianceSelectorSet()` pair, checked at `economy`-or-above like the 15 alliance functions but deliberately kept OUT of `ALLIANCE_SIGNATURES` (mirrors `guard.py`'s `_ACS_ALLIANCE_FUNCTIONS` carve-out, so the pre-existing alliance cross-layer equality never breaks). A new `resolveAllowAcsDefense` (`policy.ts`) mirrors `resolveAllowCombat`/`resolveAllowAlliance` exactly. |
...
| `cli.ts` | 329 | The `walletctl` command surface: `status`, `verify-abi`, `build`, `simulate`, `send`, `receipt`. `send` without `--confirm` always exits non-zero and prints exactly what it would have sent — the same code path as a real send, short-circuited right before `provider.signAndSend`. **The ACS defense coordination feature** added a `simulate --json` flag, emitting a single `{ok, revertReason, error, decoded}` JSON line instead of the existing plain-text output — `decoded` is the new capability (via `abi.ts`'s `decodeSimulateReturnData`), letting a caller read a `view` function's actual return value; plain-text output is unchanged when the flag is omitted. |
...
6. guard                        →  guard.py: all 25 gates, full verdict list, one Decision
...
exceptions: **Alliance Depot** (still no generator references it — this is a *building*
whose bonus feeds the alliance system's counterplay-fuel formulas; it's unrelated to the
alliance feature's own 15 *membership* functions, which are override-only and were never
going to get a `candidates.py` generator regardless, per that feature's own design) and
**Rift Stabilizer** (no generator references it either; `docs/COVERAGE.md`
...
a second test is exactly how the first one drifted. **The ACS defense coordination
feature added two more instances of the identical pattern**, each with its own diffed
pair rather than folded into an existing one: `_ACS_MISSION_TYPES`/`ACS_MISSION_TYPES`
(a third, independent mission-type set, gated on a different flag than combat's), and
`_DEFENSE_HOLD_ONLY_FUNCTIONS`/`DEFENSE_HOLD_SIGNATURES` plus `_ACS_ALLIANCE_FUNCTIONS`/
`ACS_ALLIANCE_SIGNATURES` (the same function-name carve-out shape `_COMBAT_ONLY_FUNCTIONS`
already established for Missile, one for `launchDefenseHold` and one for
`openDefenseIntent`). An adversarial review pass caught, before any code shipped, that a
...
| API routes, formulas, canonical enums, the strategy derivation, contract-write traps, guardrails, scheduling, the `vd tick --action` manual override, the incoming-attack/resolved-battle/debris radar, attack/missile/colonize/foreign-harvest/ladder-band opportunities | `skills/veydrift-agent/references/*.md` |
