...
| `opportunities.py` | ~140 | **New.** Attack/missile/colonize/foreign-harvest/transport candidates, surfaced independent of `plan.py`'s ladder outcome. `plan_next_action` is a straight-line early-return chain — a lower-priority band's candidate is never even generated once a higher band wins that tick, regardless of how long the relevant policy flag has been on. `scan_opportunities` calls the same `candidates.py` generators (`generate_attack_candidates`/`generate_missile_candidates`/`generate_colonize_candidates`/`generate_foreign_harvest_candidates`) a second time, once per owned planet via `plan._target_planets` (reused, not reimplemented), independent of the ladder. Zero changes to `plan.py`/`candidates.py`/`guard.py`: every generator already self-gates on its own policy flag internally (`allow_combat`/`allow_fleet_noncombat`/`strategy.colonize`), so this module needs no gating logic of its own — a policy with every flag off produces an empty report at negligible cost. No persisted state (an opportunity is a current-state fact, not a one-time event) and no unconditional `strategy.md` narration (unlike radar's findings, which are naturally transient, an opportunity can stay true for many ticks — forcing a line every tick would spam the log for something that hasn't changed); the tick report's `opportunities:` line and `proposals.jsonl`'s `"opportunities"` key are the only new surfaces. `vd tick` only, no standalone CLI. **The ACS defense coordination feature added a fifth family, `transport`**, via a dedicated `_scan_transport` call (`generate_transport_candidates` takes `target_planets` as a required positional 4th argument, unlike the other four generators' keyword-only shape, so it doesn't fit `_scan_planet`'s dispatch dict) — Transport's write path already existed and needed no new capability, only surfacing; Deploy remains excluded. See `references/opportunities.md`. |
...
| API routes, formulas, canonical enums, the strategy derivation, contract-write traps, guardrails, scheduling, the `vd tick --action` manual override, the incoming-attack/resolved-battle/debris radar, attack/missile/colonize/foreign-harvest opportunities | `skills/veydrift-agent/references/*.md` |
